...
public:
  ...
  // This function would override the one in Source/Core/InputCommon/ControllerInterface/CoreDevice.h
  // But it doesn't do shit since the only block of code is not compiled
  // Also, it has been updated to return DeviceRemoval
  // Soooo I'm not overriding it
...
void Init()
{
  ...
  int port_max = (Core::System::GetInstance().IsWii() && Libretro::Options::altGCPorts) ? 8 : 4;
...
{
  if (((!Core::System::GetInstance().IsWii() || !Libretro::Options::altGCPorts) && port > 3) || port > 7)
    return;
  ...
  {
    if (Core::System::GetInstance().IsWii() && port < 4)
      Config::SetCurrent(Config::GetInfoForWiimoteSource(port), WiimoteSource::None);

    if (!Core::System::GetInstance().IsWii() || !Libretro::Options::altGCPorts)
    {
      Config::SetCurrent(Config::GetInfoForSIDevice(port), SerialInterface::SIDEVICE_NONE);
      si.ChangeDevice(Config::Get(Config::GetInfoForSIDevice(port)), port);
    }
    else if (port > 3)
    {
      Config::SetCurrent(Config::GetInfoForSIDevice(port-4), SerialInterface::SIDEVICE_NONE);
      si.ChangeDevice(Config::Get(Config::GetInfoForSIDevice(port-4)), port-4);
    }
  }
  else if (!Core::System::GetInstance().IsWii() || device == RETRO_DEVICE_GC_ON_WII || port > 3)
  {
    if (device == RETRO_DEVICE_GC_ON_WII) // Disconnect Wii device if we're using GC controller as device type to avoid conflict
      Config::SetCurrent(Config::GetInfoForWiimoteSource(port), WiimoteSource::None);

    Config::SetCurrent(Config::GetInfoForSIDevice(port > 3 ? port - 4 : port), SerialInterface::SIDEVICE_GC_CONTROLLER);
  ...
  for (int i = 0; i < port_max; i++)
  {
    ...
    default:
      if (!Core::System::GetInstance().IsWii() || i > 3)
      {
        desc = Libretro::Input::descGC;
      }
      else
